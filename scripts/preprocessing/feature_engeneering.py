...
# def add_item_name_and_service_classification(df: pd.DataFrame):
...
def prepare_test(base_train: pd.DataFrame, train_df: pd.DataFrame, test_df: pd.DataFrame, encoder_dir: str = "encoders", last_month: int = 34):
    ...
    test = add_date_features(test, is_train=False, last_month=last_month)
    ...
    test = add_binary_service_or_item(test)
    test = add_global_category_and_city(test)
    ...
    test = encode_categoricals_transform(test, encoder_dir)
...
def do_feature_engeneering(train: pd.DataFrame, test:pd.DataFrame, encoder_dir: str = "encoders", last_month: int = 34):
    ...
    train_df = add_binary_service_or_item(train)
    ...
    train_df = add_global_price_features(train_df)
    encode_categoricals_fit(train_df)
    train_df = encode_categoricals_transform(train_df)
    
    test_df = prepare_test(train, train_df, test, encoder_dir=encoder_dir, last_month=last_month)
