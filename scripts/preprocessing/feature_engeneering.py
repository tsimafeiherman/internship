...
class FeatureEngineer:
    ...
    def add_sparse_zero_rows_fast(df: pd.DataFrame):
        ...
        merged = expanded_df.merge(
            df, on=["date_block_num", "shop_id", "item_id"], how="left"
        )
        ...
        merged = merged.sort_values(["shop_id", "item_id", "date_block_num"])

        merged[fill_cols] = (
            merged.groupby(["shop_id", "item_id"])[fill_cols].ffill().bfill()
        )
    ...
    def add_global_price_features(df: pd.DataFrame):
        ...
        df["is_expensive"] = (df.item_price_global_mean > expensive_threshold).astype(
            int
        )
    ...
    def prepare_test(
        ...
        last_month_data = train_df[train_df["date_block_num"] == (last_month - 1)][
            [
                "shop_id",
                "item_id",
                "lag_1",
        ...
        category_price_stats = train_df[
            [
                "global_category",
                "category_price_global_mean",
                "category_price_global_median",
            ]
        ].drop_duplicates(subset="global_category")
        test["global_category"] = test["global_category"].astype(str)
        category_price_stats["global_category"] = category_price_stats[
            "global_category"
        ].astype(str)
        ...
        test["price_ratio_to_category"] = (
            test["item_price_global_mean"] / test["category_price_global_mean"]
        )
        ...
        test["is_expensive"] = (
            test["item_price_global_mean"] > expensive_threshold
        ).astype(int)
        test["is_cheap"] = (test["item_price_global_mean"] < cheap_threshold).astype(
            int
        )
    ...
    def do_feature_engeneering(
        ...
        test_df = FeatureEngineer.prepare_test(
            train, train_df, test, encoder_dir=encoder_dir, last_month=last_month
        )
