...
def clear_items(items: pd.DataFrame) -> pd.DataFrame:
    ...
    if len(name_duplicates) > 0:
        print(f"Обнаружено {len(name_duplicates)} записей с дублирующимися названиями после чистки!")
        print("Примеры дубликатов:")
        print(name_duplicates[['item_id', 'item_name']].head(10))
        
        duplicate_names = name_duplicates.groupby('item_name').size().reset_index(name='count')
        ...
        print("\nВероятная причина:")
        print("- Разные товары с одинаковым названием после очистки от спецсимволов")
...
def clear_shops(shops: pd.DataFrame) -> pd.DataFrame:
    ...
    df.shop_name = df.shop_name.str.replace(r'[^a-zA-Zа-яА-Я0-9\s\.\-\(\)]', '', regex=True)
    df.shop_name = df.shop_name.str.replace(r'\s+', ' ', regex=True)
    
    name_duplicates = df[df.duplicated(subset="shop_name",keep=False)]
    
    if len(name_duplicates) > 0:
        print(f"Обнаружено {len(name_duplicates)} записей с дублирующимися названиями магазинов!")
...
def clear_sales(sales: pd.DataFrame, verbose: bool = True) -> pd.DataFrame:
    ...
    if verbose:
        ...
        print(f"  Отрицательных количеств осталось: {(df['item_cnt_day'] < 0).sum()}")
...
def clear_all(
    ...
    sales_train = clear_sales(sales_train, verbose)
...
def merge_dataframes(
    ...
    items: pd.DataFrame,
    sales_train: pd.DataFrame,
    shops: pd.DataFrame) -> tuple[pd.DataFrame, dict]:
    ...
    if merge_report["missing_items_count"] > 0:
        merge_report["missing_item_examples"] = missing_items[['item_id', 'shop_id', 'item_cnt_day']].head(10).to_dict(orient="records")
    
    missing_categories = items[~items['item_category_id'].isin(item_categories['item_category_id'])]
    merge_report["missing_categories_count"] = len(missing_categories)
    if merge_report["missing_categories_count"] > 0:
        merge_report["missing_category_examples"] = missing_categories[['item_id', 'item_category_id']].head(10).to_dict(orient="records")
    ...
    if merge_report["missing_shops_count"] > 0:
        merge_report["missing_shop_examples"] = missing_shops[['shop_id', 'item_id', 'item_cnt_day']].head(10).to_dict(orient="records")
    
    merge_report["has_issues"] = (
        merge_report["missing_items_count"] > 0 or
        merge_report["missing_categories_count"] > 0 or
        merge_report["missing_shops_count"] > 0 or
        merge_report["null_columns_count"] > 0
    )
