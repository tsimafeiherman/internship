...
class FinalData():
    ...
    def save_artifacts(self, artifacts_dir: str = "artifacts"):
        ...
        # item_history
        item_history_cols = ['item_id', 'first_month', 'last_month', 'month_with_sales', 'total_sales', 'avg_sales']
        item_history = train_df[item_history_cols].drop_duplicates('item_id')
        item_history.to_csv(out / "item_history.csv", index=False)

        # price_stats
        price_stats = train_df[['item_id', 'item_price_global_mean', 'item_price_global_median', 'item_price_global_std']].drop_duplicates('item_id')
        price_stats.to_csv(out / "price_stats.csv", index=False)

        # category_price_stats
        cat_price_stats = train_df[['global_category', 'category_price_global_mean', 'category_price_global_median']].drop_duplicates('global_category')
        ...
        last_lags = train_df[train_df['date_block_num'] == self.last_month - 1][
            ['shop_id', 'item_id', 'lag_1', 'lag_2', 'lag_3', 'lag_12', 'rolling_mean_3', 'had_sales_lag1']
        ]
        ...
        thresholds = {'expensive_threshold': expensive_threshold, 'cheap_threshold': cheap_threshold}
    ...
    def load_artifacts(cls, artifacts_dir: str):
        ...
        obj.items_full = obj.items.merge(obj.item_categories, on="item_category_id", how="left")
        ...
        obj.category_price_stats = pd.read_csv(artifacts_dir / "category_price_stats.csv")
    ...
    def predict(self, shop_id: int, item_id: int) -> float:
        ...
        shop_name = shop_row.iloc[0]["shop_name"] if not shop_row.empty else "UNKNOWN_SHOP"
        ...
        self.category_price_stats['global_category'] = self.category_price_stats['global_category'].astype(str)
        ...
        df["price_ratio_to_category"] = df["item_price_global_mean"] / df["category_price_global_mean"]
        df["price_ratio_to_category"] = df["price_ratio_to_category"].fillna(1)
        df["is_expensive"] = (df["item_price_global_mean"] > self.expensive_threshold).astype(int)
        df["is_cheap"] = (df["item_price_global_mean"] < self.cheap_threshold).astype(int)

        df = df.drop(columns=['ID', 'shop_name', 'item_name', 'item_category_name'], errors='ignore')
